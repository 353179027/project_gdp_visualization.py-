...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    for pcc in plot_countries :#从plot_countries中取出国家代码
        if plot_countries[pcc] in gdp_countries : 	#判断某国家是否在世行有数据
            d[pcc] = plot_countries[pcc]
            gdp_countries.pop(gdp_countries.index(plot_countries[pcc]))#将没有数据的国家代码取出
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
    ...
    s2 = set ()
    yg=[]
    for pcc in plot_countries:
        for icc in gdpinfo :
                        
            if gdpinfo[icc]["Country Name"] == plot_countries[pcc] :  #判断是否存在数据              
                gnb = ''
                for y in range(1960,2016) :
                    gnb += gdpinfo[icc]['%d'%y]#将所有的年份的GDP数据相加返还
                if gnb == '' :#判断是否有gdp数据
                    s1.add(pcc)#得到在世行GDP数据中完全没有记录的绘图库国家代码的集合
                else :
                    if gdpinfo[icc][year]!='':#当年有GDP数据
                        gn = math.log10(float((gdpinfo[icc][year])))#GDP结果需转换为以10为基数的对数格式
                        cyg[pcc] = gn
                    else :
                        s2.add(pcc)
                continue
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    A_countries,B_countries,C_countries = build_map_dict_by_name(gdpinfo, plot_countries, year)#用pygal绘制地图
...
while float(year) < 1960 or float(year) > 2015 :#判断是否在范围
